...
void MCTS::RolloutSearch()
{
    ...
    std::cout << "number of simulations: " << Params.NumSimulations << std::endl;
...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action)
{
    ...
    if (!terminal)
    {
        ...
        {
//        	StatTreeDepth.Add(TreeDepth);
            delayedReward = Rollout(state);
//            delayedReward = RolloutAstar(state);
        }
        TreeDepth--;
    }
...
double MCTS::RolloutAstar(STATE& state)
{
    Status.Phase = SIMULATOR::STATUS::ROLLOUT;
    if (Params.Verbose >= 3)
        cout << "Starting rollout" << endl;
...
//    random_generator.seed(std::time(0));
    ...
    maze m = random_maze(x, y, robotstate.AgentPos.X, robotstate.AgentPos.Y);

    bool isSolved = m.solve();
    std::stack<int> solution_stack;

    isSolved = false;

    if (isSolved) {
//        std::cout << "The maze is solvable." << std::endl;
//        std::cout << m << std::endl;
        solution_stack = m.get_stack_solution();

        int stack_size = solution_stack.size();

        for (numSteps = 0; numSteps < Params.MaxDepth && !terminal; numSteps++)
//        while(!terminal)
        {
            int observation;
            double reward;
            int action;
            if (numSteps < stack_size) {
                action = solution_stack.top();
                solution_stack.pop();
...
maze MCTS::random_maze(int x, int y, int startX, int startY)
{
    maze m(x, y, startX, startY);

    int barriers = 58;
    int iteration = 1;
    while (iteration++ < 15) {
        vertex_descriptor u = vertex(barriers, m.m_grid);
        barriers += 29;
    ...
    vertex_descriptor u = vertex(barriers, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-1, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-2, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-3, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-4, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-5, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-6, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-7, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-8, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-9, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-10, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-11, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-12, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-13, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-14, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-15, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-16, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-17, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-18, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-19, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-20, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-21, m.m_grid);
    m.m_barriers.insert(u);
    u = vertex(barriers-22, m.m_grid);
    m.m_barriers.insert(u);
