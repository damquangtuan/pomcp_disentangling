...
void EXPERIMENT::Run()
{
    ...
    double treeDepthAll = 0.0;
    double getToGoalAll = 0.0;
    double numberOfCollisions = 0.0;
    double discount = 1.0;
    bool terminal = false;
    ...
    for (t = 0; t < ExpParams.NumSteps; t++)
    {
        ...
        double reward;
        bool isCollision;
        int x, y, treeDepth, isGoal;
        int action = mcts.SelectAction();

        auto results = Real.StepReal(*state, action, observation, reward);
        terminal = std::get<0>(results);
        isCollision = std::get<1>(results);
        x = std::get<2>(results);
        y = std::get<3>(results);

        Simulator.UpdateProbabiliticMap(x, y, action, isCollision);

        treeDepth = mcts.GetTreeDepth();
        ...
        treeDepthAll += treeDepth;
        getToGoalAll += isGoal;
        numberOfCollisions += int(isCollision);
    ...
    treeDepthAll = treeDepthAll/ExpParams.NumSteps;
    ...
    if (outOfParticles)
    {
        ...
        while (++t < ExpParams.NumSteps) {
            ...
            auto results = Real.StepReal(*state, action, observation, reward);
            terminal = std::get<0>(results);
            bool isCollision = std::get<1>(results);
            int x = std::get<2>(results);
            int y = std::get<3>(results);

            Simulator.UpdateProbabiliticMap(x, y, action, isCollision);

            int treeDepth = mcts.GetTreeDepth();

            int isGoal = int(terminal);
    ...
    Results.TreeDepth.Add(treeDepthAll);
    Results.GetToGoal.Add(getToGoalAll);
    Results.NumberOfCollision.Add(numberOfCollisions);
    ...
    cout << "TreeDepth return = " << treeDepthAll
        << ", average = " << Results.TreeDepth.GetMean() << endl;
    cout << "GetToGoal return = " << getToGoalAll
         << ", average = " << Results.GetToGoal.GetMean() << endl;
    cout << "NumberOfCollision return = " << numberOfCollisions
         << ", average = " << Results.NumberOfCollision.GetMean() << endl;
}
...
{
    cout << "Main runs" << endl;
    OutputFile << "Simulations\tGetToGoal return\tGetToGoal error\tNumberOfCollision return\tNumberOfCollision error\tRuns\tUndiscounted return\tUndiscounted error"
                  "\tDiscounted return\tDiscounted error\tTreeDepth return\tTreeDepth error\n";
    ...
    for (int i = ExpParams.MinDoubles; i <= ExpParams.MaxDoubles; i++)
    {
        ...
        cout << "Simulations = " << SearchParams.NumSimulations << endl
            << "GetToGoal return = " << Results.GetToGoal.GetMean()
            << " +- " << Results.GetToGoal.GetStdErr() << endl
            << "NumberOfCollision return = " << Results.NumberOfCollision.GetMean()
            << " +- " << Results.NumberOfCollision.GetStdErr() << endl
            ...
            << "TreeDepth return = " << Results.TreeDepth.GetMean()
            << " +- " << Results.TreeDepth.GetStdErr() << endl
            << "Time = " << Results.Time.GetMean() << endl;
        OutputFile << SearchParams.NumSimulations << "\t"
            << Results.GetToGoal.GetMean() << "\t"
            << Results.GetToGoal.GetStdErr() << "\t"
            << Results.NumberOfCollision.GetMean() << "\t"
            << Results.NumberOfCollision.GetStdErr() << "\t"
            ...
            << Results.TreeDepth.GetMean() << "\t"
            << Results.TreeDepth.GetStdErr() << "\t"
