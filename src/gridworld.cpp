...
double GRIDWORLD::sumSuccessProbFromStart(int x_current, int y_current) const{
    double reward = log(1 - ProbabilisticMap(0,0));
    if (x_current > y_current)
        for (int x = 1; x <= x_current; x++)
        {
            int y = int((y_current/x_current) * x);
        ...
        for (int y = 1; y <= y_current; y++)
        {
            int x = int((x_current/y_current) * y);
            if (ProbabilisticMap(x,y) != -1) {
                double intermediateReward = double(ProbabilisticMap(x,y));
                reward += log(1 - intermediateReward);
            }
...
double GRIDWORLD::sumSuccessProb(int x_current, int y_current) const{
    double reward = log(1 - ProbabilisticMap(x_current,y_current));
//    return reward;
...
//        printMap(false);
    ...
    if (!collidedBorder) {
        ...
        a->updateCollisionPoint(x,y,action);
//        a->hitCollision();
    ...
    switch (action)
    {
        case COORD::E_EAST: {
            ...
            if (ProbabilisticMap(x, y_tmp) != -1) ProbabilisticMap(x, y_tmp) = 0.9;

            y_tmp = (y+2) < sizeY ? (y+2) : sizeY;
            if (ProbabilisticMap(x, y_tmp) != -1) ProbabilisticMap(x, y_tmp) = 0.8;
            ...
            if (ProbabilisticMap(x_tmp, y) != -1) ProbabilisticMap(x_tmp, y) = 0.9;

            x_tmp = (x+2) < sizeX ? (x+2) : sizeX;
            if (ProbabilisticMap(x_tmp, y) != -1) ProbabilisticMap(x_tmp, y) = 0.8;

            x_tmp = (x+1) < sizeX ? (x+1) : sizeX;
            y_tmp = (y+1) < sizeY ? (y+1) : sizeY;
            if (ProbabilisticMap(x_tmp, y_tmp) != -1) ProbabilisticMap(x_tmp, y_tmp) = 0.8;
        ...
        case COORD::E_SOUTHEAST: {
            x_tmp = (x-1) > 0 ? (x-1) : 0;
            y_tmp = (y+1) < sizeY ? (y+1) : sizeY;
            if (ProbabilisticMap(x_tmp, y_tmp) != -1) ProbabilisticMap(x_tmp, y_tmp) = 0.9;
...
{
//    ASTAR *aTemp = new ASTAR(sizeX, sizeY);
//    a->setStartPosition(x_current, y_current);
//    std::vector<int_pi> solutionReturn = a->solveReturn();
//
//    vector<int_pi>::reverse_iterator it;
//    double rewardTotal = 0;
//    double discountTotal = 1;
//    for (it = solutionReturn.rbegin(); it != solutionReturn.rend(); ++it)
//    {
//        rewardTotal += log(1 - ProbabilisticMap(it->first, it->second)) * discountTotal;
...
//    return 0;
    ...
    return sumSuccessProb(x_current, y_current);// + sumSuccessProbFromStart(x_current, y_current);
//
