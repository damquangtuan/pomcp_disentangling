...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("test", "run unit tests")
        ("problem", value<string>(&problem), "problem to run (battleship/pocman/network/rocksample/tag/gridworld)")
        ...
        ("explorationconstant", value<double>(&searchParams.Cp), "exploration constant")
        ;
    ...
    {
        real = new ROCKSAMPLE_REAL(size, number);
        simulator = new ROCKSAMPLE(size, number, searchParams.Cp);
    }
    else if (problem == "tag")
    {
        real = new TAG(number);
    ...
    else if (problem == "gridworld")
    {
        SIMULATOR* real = 0;
        SIMULATOR* simulator = 0;

        real = new GRIDWORLD(20, 30, searchParams.Cp, true);
        simulator = new GRIDWORLD(20, 30, searchParams.Cp, false);

        char outputfileBuff[200];
        sprintf(outputfileBuff, "output.txt_gridworld_%d_%d_numrun_%d_sim_%d_rolloutknowledge_%d_treeknowledge_%d_constant_%f"
                , size, number, expParams.NumRuns, expParams.MaxDoubles, knowledge.RolloutLevel, knowledge.TreeLevel,
                searchParams.Cp);
