...
std::tuple<bool, bool, int, int> NETWORK::StepReal(STATE& state, int action, int& observation, double& reward)
{
    return make_tuple(false, false, -1, -1);
}

void NETWORK::UpdateProbabiliticMap(int x, int y, int action, bool isCollision){}

void NETWORK::ClearMap(){}

std::tuple<bool, std::vector<int>> NETWORK::Rollout(STATE& state)
{
