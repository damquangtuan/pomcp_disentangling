...
ROCKSAMPLE::ROCKSAMPLE(int size, int rocks, double Cp)
:   Grid(size, size),
    Size(size),
    NumRocks(rocks),
    SmartMoveProb(0.95),
...
std::tuple<bool, bool, int, int> ROCKSAMPLE::StepReal(STATE& state, int action, int& observation, double& reward)
{
    return std::make_tuple(false, false, -1, -1);
}

void ROCKSAMPLE::UpdateProbabiliticMap(int x, int y, int action, bool isCollision)
{

}

void ROCKSAMPLE::ClearMap()
{

}

std::tuple<bool, std::vector<int>> ROCKSAMPLE::Rollout(STATE& state)
{
...
ROCKSAMPLE_REAL::ROCKSAMPLE_REAL(int size, int rocks)
:   Grid(size, size),
    Size(size),
    NumRocks(rocks),
    SmartMoveProb(0.95),
...
void ROCKSAMPLE_REAL::InitGeneral()
{
    HalfEfficiencyDistance = 20;
    StartPos = COORD(0, Size / 2);
    RandomSeed(0);
...
void ROCKSAMPLE_REAL::Init_11_11()
{
    // Equivalent to RockSample_11_11.pomdp(x)
    cout << "Using special layout for rocksample(11, 11)" << endl;
...
STATE* ROCKSAMPLE_REAL::Copy(const STATE& state) const
{
    const ROCKSAMPLE_STATE& rockstate = safe_cast<const ROCKSAMPLE_STATE&>(state);
    ROCKSAMPLE_STATE* newstate = MemoryPool.Allocate();
    *newstate = rockstate;
...
void ROCKSAMPLE_REAL::Validate(const STATE& state) const
{
    const ROCKSAMPLE_STATE& rockstate = safe_cast<const ROCKSAMPLE_STATE&>(state);
    assert(Grid.Inside(rockstate.AgentPos));
}

STATE* ROCKSAMPLE_REAL::CreateStartState() const
{
    ROCKSAMPLE_STATE* rockstate = MemoryPool.Allocate();
    rockstate->AgentPos = StartPos;
    rockstate->Rocks.clear();
...
void ROCKSAMPLE_REAL::FreeState(STATE* state) const
{
    ROCKSAMPLE_STATE* rockstate = safe_cast<ROCKSAMPLE_STATE*>(state);
    MemoryPool.Free(rockstate);
}

bool ROCKSAMPLE_REAL::Step(STATE& state, int action,
    int& observation, double& reward) const
{
    ROCKSAMPLE_STATE& rockstate = safe_cast<ROCKSAMPLE_STATE&>(state);
    reward = 0;
...
std::tuple<bool, bool, int, int> ROCKSAMPLE_REAL::StepReal(STATE& state, int action, int& observation, double& reward)
{
    return std::make_tuple(false, false, -1, -1);
}

void ROCKSAMPLE_REAL::UpdateProbabiliticMap(int x, int y, int action, bool isCollision)
{

}

void ROCKSAMPLE_REAL::ClearMap()
{

}

std::tuple<bool, std::vector<int>> ROCKSAMPLE_REAL::Rollout(STATE& state)
{
    return make_tuple(true, std::vector<int>(0));
}

bool ROCKSAMPLE_REAL::LocalMove(STATE& state, const HISTORY& history,
    int stepObs, const STATUS& status) const
{
    ROCKSAMPLE_STATE& rockstate = safe_cast<ROCKSAMPLE_STATE&>(state);
    int rock = Random(NumRocks);
...
void ROCKSAMPLE_REAL::GenerateLegal(const STATE& state, const HISTORY& history,
    vector<int>& legal, const STATUS& status) const
{
...
void ROCKSAMPLE_REAL::GeneratePreferred(const STATE& state, const HISTORY& history,
    vector<int>& actions, const STATUS& status) const
{
...
int ROCKSAMPLE_REAL::GetObservation(const ROCKSAMPLE_STATE& rockstate, int rock) const
{
    double distance = COORD::EuclideanDistance(rockstate.AgentPos, RockPos[rock]);
    double efficiency = (1 + pow(2, -distance / HalfEfficiencyDistance)) * 0.5;
...
int ROCKSAMPLE_REAL::SelectTarget(const ROCKSAMPLE_STATE& rockstate) const
{
    int bestDist = Size * 2;
    int bestRock = -1;
    for (int rock = 0; rock < NumRocks; ++rock)
...
void ROCKSAMPLE_REAL::DisplayBeliefs(const BELIEF_STATE& beliefState,
    std::ostream& ostr) const
{
}

void ROCKSAMPLE_REAL::DisplayState(const STATE& state, std::ostream& ostr) const
{
    const ROCKSAMPLE_STATE& rockstate = safe_cast<const ROCKSAMPLE_STATE&>(state);
    ostr << endl;
    for (int x = 0; x < Size + 2; x++)
...
void ROCKSAMPLE_REAL::DisplayObservation(const STATE& state, int observation, std::ostream& ostr) const
{
    switch (observation)
    {
    case E_NONE:
...
void ROCKSAMPLE_REAL::DisplayAction(int action, std::ostream& ostr) const
{
    if (action < E_SAMPLE)
        ostr << COORD::CompassString[action] << endl;
    if (action == E_SAMPLE)
