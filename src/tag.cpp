...
std::tuple<bool, bool, int, int> TAG::StepReal(STATE& state, int action, int& observation, double& reward)
{
    return std::make_tuple(false, false, -1, -1);
}

void TAG::UpdateProbabiliticMap(int x, int y, int action, bool isCollision)
{
...
std::tuple<bool, std::vector<int>> TAG::Rollout(STATE& state)
{
